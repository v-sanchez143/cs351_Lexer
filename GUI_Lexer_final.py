...
def CutOneLineTokens(code):
    ...
    while len(new_code) > 0:  # loops until int count has reached the last index of code str
        ...
        elif re.match(r'"[^"]*"', new_code):
            tt = "string_literal"
            token = re.match(r'"[^"]*"', new_code)
...
def parser(tokens):
    ...
    elif "keyword" in first_token and ("float" in first_token or "int" in first_token):
        output.append("Processing Declaration statement")
        output.extend(assign_exp(tokens))
    else:
...
def print_exp(tokens):
    ...
    if "identifier" in tokens[i] or "string_literal" in tokens[i]:
...
def assign_exp(tokens):
    output = []
    i = 0
    output.append("Start <assign_exp>")

    #float/int
    if "float" in tokens[i] or "int" in tokens[i]:
        output.append(f"Match {tokens[i]}")
        i += 1
    else:
        output.append("Error: expected type")
        return output
    ...
    output.append("Processing expression")
    ...
    while i < len(tokens) and ";" not in tokens[i]:
        ...
        i += 1
    output.extend(math_exp(expr_tokens))
